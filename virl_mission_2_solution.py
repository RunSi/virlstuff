...
PROJUSERNAME = project_name
PROJPASSWORD = project_name

# TODO define a check project function
# TODO create code to check whether project_name exists.  If does not exist
# TODO then execute code below, otherwise skip.

def check_project(url, username, password):
    ...
    response = requests.get(project_url, auth=(username, password), headers=headers).json()

    found = 0
    for i, names in enumerate(response['projects']):
        # print(response['projects'][i]['name'])
        projname = (response['projects'][i]['name'])
...
def packet_capture(url, username, password, sim):
    ...
    # dictionary  - This iteration will create an issue as i is a counter and not the interface ID, hence the user
    # response on the while loop will be wrong.  Need to give the interface ID and not i !!
    for iface in iface_response[sim][mission_node]:
        # Check to see if interface is not management and then assign it
        if iface != 'management':
            ifaces[iface] = iface_response[sim][mission_node][iface]["name"]
            print(
...
def main():
    ...
    found = check_project(VIRL_URL, GUESTUSERNAME, GUESTPASSWORD)

    if found == 0:
        create_project_user(VIRL_URL, GUESTUSERNAME, GUESTPASSWORD)
    else:
        print('\nProject ' + project_name + ' exists no need to create')


    # start start_sim(url, username, password) function and assign the result
    # to the variable
    simulation = start_sim(VIRL_URL, PROJUSERNAME, PROJPASSWORD)
    ...
    packet_capture(VIRL_URL, PROJUSERNAME, PROJPASSWORD, simulation)
    ...
    stop_simulation(VIRL_URL, PROJUSERNAME, PROJPASSWORD, simulation)
